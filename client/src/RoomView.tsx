...
export function RoomView({ projectHash, onBack }: RoomViewProps) {
  ...
  const editor = useEditorActions(getOfficeState, editorState, projectHash)

  const isEditDirty = useCallback(() => editor.isEditMode && editor.isDirty, [editor.isEditMode, editor.isDirty])

  const { agents, agentTools, agentStatuses, subagentTools, subagentCharacters, layoutReady, loadedAssets } = useWebSocket(projectHash, getOfficeState, editor.setLastSavedLayout, isEditDirty)

  const [isDebugMode, setIsDebugMode] = useState(false)

  const handleToggleDebugMode = useCallback(() => setIsDebugMode((prev) => !prev), [])

  // In the web version, clicking an agent only highlights it (no terminal to focus)
  const handleClick = useCallback((_agentId: number) => {
    // View-only: selection/follow is handled inside OfficeCanvas
  }, [])
  ...
  // Export layout: serialize current OfficeState layout to JSON string
  const handleExportLayout = useCallback((): string | null => {
    const os = getOfficeState()
    const layout = os.getLayout()
    return JSON.stringify(layout, null, 2)
  }, [getOfficeState])

  // Import layout: rebuild from imported layout data
  const handleImportLayout = useCallback((layoutData: unknown) => {
    const os = getOfficeState()
    // The SettingsModal already validates version + tiles
    os.rebuildFromLayout(layoutData as Parameters<typeof os.rebuildFromLayout>[0])
    editor.setLastSavedLayout(layoutData as Parameters<typeof editor.setLastSavedLayout>[0])
    editor.handleSave()
  }, [getOfficeState, editor])
