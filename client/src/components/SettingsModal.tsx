...
export function SettingsModal({ isOpen, onClose, isDebugMode, onToggleDebugMode, onExportLayout, onImportLayout }: SettingsModalProps) {
    ...
    reader.onload = () => {
      try {
        ...
        if (
          ...
          'tiles' in (parsed as Record<string, unknown>)
        ) {
          onImportLayout(parsed)
          onClose()
        } else {
          console.error('[Settings] Invalid layout file: missing version or tiles')
        }
      } catch (err) {
