	...
	try {
		...
		if (record.type === 'assistant' && Array.isArray(record.message?.content)) {
			...
			if (hasToolUse) {
				...
				if (hasNonExemptTool) {
					startPermissionTimer(agentId, agents, permissionTimers, PERMISSION_EXEMPT_TOOLS, messageSink);
				}
				...
				startWaitingTimer(agentId, TEXT_IDLE_DELAY_MS, agents, waitingTimers, messageSink);
			}
		} else if (record.type === 'progress') {
			processProgressRecord(agentId, record, agents, waitingTimers, permissionTimers, messageSink);
			...
			if (Array.isArray(content)) {
				...
				if (hasToolResult) {
					...
					cancelWaitingTimer(agentId, waitingTimers);
					clearAgentActivity(agent, agentId, permissionTimers, messageSink);
					agent.hadToolsInTurn = false;
				}
...
function processProgressRecord(
	...
	agents: Map<string, AgentState>,
	_waitingTimers: Map<string, ReturnType<typeof setTimeout>>,
	permissionTimers: Map<string, ReturnType<typeof setTimeout>>,
	messageSink: MessageSink | undefined,
): void {
	const agent = agents.get(agentId);
